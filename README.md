...
### 4. 見出し画像 (Eyecatch)
- **ランダム選択**: プロジェクト直下の `eyecatch` フォルダに画像ファイル（.png, .jpg, .jpeg, .webp）を入れておくと、その中からランダムに1枚が選ばれて使用されます。
- **固定画像**: `eyecatch` フォルダがない、または空の場合、ルートディレクトリの `eyecatch.png` が使用されます。
- **画像生成 (現在無効)**: 以前の自動生成機能は、アップロード機能の安定化のため一時的に無効化されています。
